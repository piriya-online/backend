...
function loadShop(){
	...
	$.post('https://api.remaxthailand.co.th/shop/microShop/name', {
		...
		if (data.success) {
			...
			for( i=0; i<data.result.length; i++ ) {
				...
				$('#cbb-microshop').append('<option value="'+result.id+'"'+(Cookies.get('shopSelected') == result.id ? ' selected' : '')+'>'+result.name+'</option>');
			}
			//$('#cbb-microshop option:eq(0)').attr('selected', 'selected');
