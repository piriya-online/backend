...
$(function() {
    ...
    $('#btn-agingPdf').click(function() {
        window.open('https://api.remaxthailand.co.th/report/aging/' + shopID, '_blank');
        return false;
    });
    ...
    $('#btn-runratePdf').click(function() {
        window.open('https://api.remaxthailand.co.th/report/run_rate/' + shopID, '_blank');
        return false;
    });
	...
	$(document).on('keydown', '#password-login_content', function(e){
		var key = e.charCode || e.keyCode || 0;
		if (key == 13) {
			if(memberId == '150701013' && $('#password-login_content').val() == '268268'){
				$('#dv-login_content').hide();
				$('#dv-content').show();
			} else if(memberId == '160103303' && $('#password-login_content').val() == '268268'){
				$('#dv-login_content').hide();
				$('#dv-content').show();
			} else if(memberId == '171110960' && $('#password-login_content').val() == '268268'){
				$('#dv-login_content').hide();
				$('#dv-content').show();
			} else {
				$('#lbl-alert_login').html('รหัสผ่านไม่ถูกต้อง')
			}
		}
	});
	...
	$(document).on('click', '#btn-submit_ownCode', function () {
		if($('#txt-ownCode').val() == ownCode){
			ownCoded = true;
			$('.wait2').show();
			$('.wait1').hide();
			$('#dv-unlock_owner').hide();
			$('#btn-owner').hide();
		}else{
			ownCoded = false;
			$('#txt-message').html('รหัสไม่ถูกต้อง');
		}
	});
});
        ...
        if (data.success) {
            if (data.result.length > 0) {
                shopID = data.result[0].shop;
					memberId = data.result[0].member;
					ownCode = data.result[0].ownCode;
					if(memberId == '161106730' || memberId == '160103303' || memberId == '150701013' || memberId == '171110960' ){
						$('#dv-login_content').show();
						$('#dv-content').hide();
					}else{
						$('#dv-login_content').hide();
						$('#dv-content').show();
    ...
    $.post('https://api.remaxthailand.co.th/shop/salesSummary', {
        ...
        if (data.success) {
            ...
            for (i = 0; i < json.length; i++) {
				...
				html1 += '<tr>';
                html1 += '<td width="10" class="text-center" valign="middle">' + moment(result.date).utcOffset(0).format('DD/MM/YYYY') + '</td>';
                html1 += '<td width="50" class="sumBill1 text-center" valign="middle" data-val= ' + result.billQty + '>' + ((result.billQty == 0) ? '-' : numberWithCommas(result.billQty)) + '</td>';
                html1 += '<td width="200" class="sumSales1 text-center active" valign="middle" data-val= ' + result.sales + '>' + ((result.sales == 0) ? '-' : numberWithCommas(result.sales)) + '</td>';
                html1 += '<td width="200" class="sumCredit1 text-center active" valign="middle" data-val= ' + result.credit + '>' + ((result.credit == 0) ? '-' : numberWithCommas(result.credit)) + '</td>';
				html1 += '<td width="200" class="sumDiscountCash1 text-center active" valign="middle" data-val= '+result.discountCash+'>'+((result.discountCash == 0) ? '-' : numberWithCommas(result.discountCash))+'</td>';
                html1 += '<td width="200" class="sumTotal1 text-center text-bold active" valign="middle" data-val= ' + result.total + '>' + ((result.total == 0) ? '-' : numberWithCommas(result.total)) + '</td>';
                html1 += '<td width="200" class="sumClaim1 text-center" valign="middle" data-val= ' + result.claim + '>' + ((result.claim == 0) ? '-' : numberWithCommas(result.claim)) + '' + ((result.claimQty == 0) ? '' : '(' + numberWithCommas(result.claimQty) + ')') + '</td>';
                html1 += '</tr>';
            }

			$('#tb-result2 tbody').html(html);
			$('#tb-result1 tbody').html(html1);
            loadAccumulated();
            loadReceivable();
            summary();
            if (data.result.length == 0) {
    ...
    $.post('https://api.remaxthailand.co.th/shop/accumulated', {
        ...
        if (data.success) {
            ...
            if (values[0].length > 0) {
				$('#sales1-jan').html(((values[0][0].jan == 0) ? '-' : numberWithCommas(values[0][0].jan)));
                $('#sales1-feb').html(((values[0][0].feb == 0) ? '-' : numberWithCommas(values[0][0].feb)));
                $('#sales1-mar').html(((values[0][0].mar == 0) ? '-' : numberWithCommas(values[0][0].mar)));
                $('#sales1-apr').html(((values[0][0].apr == 0) ? '-' : numberWithCommas(values[0][0].apr)));
                $('#sales1-may').html(((values[0][0].may == 0) ? '-' : numberWithCommas(values[0][0].may)));
                $('#sales1-jun').html(((values[0][0].jun == 0) ? '-' : numberWithCommas(values[0][0].jun)));
                $('#sales1-jul').html(((values[0][0].jul == 0) ? '-' : numberWithCommas(values[0][0].jul)));
                $('#sales1-aug').html(((values[0][0].aug == 0) ? '-' : numberWithCommas(values[0][0].aug)));
                $('#sales1-sep').html(((values[0][0].sep == 0) ? '-' : numberWithCommas(values[0][0].sep)));
                $('#sales1-oct').html(((values[0][0].oct == 0) ? '-' : numberWithCommas(values[0][0].oct)));
                $('#sales1-nov').html(((values[0][0].nov == 0) ? '-' : numberWithCommas(values[0][0].nov)));
                $('#sales1-dec').html(((values[0][0].dec == 0) ? '-' : numberWithCommas(values[0][0].dec)));

                $('#bill1-jan').html(((values[3][0].jan == 0) ? '-' : numberWithCommas(values[3][0].jan)));
                $('#bill1-feb').html(((values[3][0].feb == 0) ? '-' : numberWithCommas(values[3][0].feb)));
                $('#bill1-mar').html(((values[3][0].mar == 0) ? '-' : numberWithCommas(values[3][0].mar)));
                $('#bill1-apr').html(((values[3][0].apr == 0) ? '-' : numberWithCommas(values[3][0].apr)));
                $('#bill1-may').html(((values[3][0].may == 0) ? '-' : numberWithCommas(values[3][0].may)));
                $('#bill1-jun').html(((values[3][0].jun == 0) ? '-' : numberWithCommas(values[3][0].jun)));
                $('#bill1-jul').html(((values[3][0].jul == 0) ? '-' : numberWithCommas(values[3][0].jul)));
                $('#bill1-aug').html(((values[3][0].aug == 0) ? '-' : numberWithCommas(values[3][0].aug)));
                $('#bill1-sep').html(((values[3][0].sep == 0) ? '-' : numberWithCommas(values[3][0].sep)));
                $('#bill1-oct').html(((values[3][0].oct == 0) ? '-' : numberWithCommas(values[3][0].oct)));
                $('#bill1-nov').html(((values[3][0].nov == 0) ? '-' : numberWithCommas(values[3][0].nov)));
                $('#bill1-dec').html(((values[3][0].dec == 0) ? '-' : numberWithCommas(values[3][0].dec)));

                $('#qty1-jan').html(((values[4][0].jan == 0) ? '-' : numberWithCommas(values[4][0].jan)));
                $('#qty1-feb').html(((values[4][0].feb == 0) ? '-' : numberWithCommas(values[4][0].feb)));
                $('#qty1-mar').html(((values[4][0].mar == 0) ? '-' : numberWithCommas(values[4][0].mar)));
                $('#qty1-apr').html(((values[4][0].apr == 0) ? '-' : numberWithCommas(values[4][0].apr)));
                $('#qty1-may').html(((values[4][0].may == 0) ? '-' : numberWithCommas(values[4][0].may)));
                $('#qty1-jun').html(((values[4][0].jun == 0) ? '-' : numberWithCommas(values[4][0].jun)));
                $('#qty1-jul').html(((values[4][0].jul == 0) ? '-' : numberWithCommas(values[4][0].jul)));
                $('#qty1-aug').html(((values[4][0].aug == 0) ? '-' : numberWithCommas(values[4][0].aug)));
                $('#qty1-sep').html(((values[4][0].sep == 0) ? '-' : numberWithCommas(values[4][0].sep)));
                $('#qty1-oct').html(((values[4][0].oct == 0) ? '-' : numberWithCommas(values[4][0].oct)));
                $('#qty1-nov').html(((values[4][0].nov == 0) ? '-' : numberWithCommas(values[4][0].nov)));
				$('#qty1-dec').html(((values[4][0].dec == 0) ? '-' : numberWithCommas(values[4][0].dec)));
...
function summary() {
	$('#sumCash1').html('<img class="loading" alt="loading" src="/images/icons/loading.gif">');
    $('#sumCredit1').html('<img class="loading" alt="loading" src="/images/icons/loading.gif">');
	$('#sumDiscountCash1').html('<img class="loading" alt="loading" src="/images/icons/loading.gif">');
    $('#sumTotal1').html('<img class="loading" alt="loading" src="/images/icons/loading.gif">');
	$('#sumClaim1').html('<img class="loading" alt="loading" src="/images/icons/loading.gif">');
    ...
    var sumCash1 = 0;
    var sumCredit1 = 0;
	var sumDiscountCash1 = 0;
    var sumTotal1 = 0;
	var sumClaim1 = 0;
	$('.sumBill1').each(function(index) {
        var value = $(this).attr('data-val');
        if (!isNaN(value) && value.length != 0) {
            sumBill1 += parseFloat(value);
        }
    });
    $('.sumSales1').each(function(index) {
        var value = $(this).attr('data-val');
        if (!isNaN(value) && value.length != 0) {
            sumCash1 += parseFloat(value);
        }
    });
    $('.sumCredit1').each(function(index) {
        var value = $(this).attr('data-val');
        if (!isNaN(value) && value.length != 0) {
            sumCredit1 += parseFloat(value);
        }
    });
	$('.sumDiscountCash1').each(function(index) {
		var value = $(this).attr('data-val');
		if(!isNaN(value) && value.length != 0) {
			sumDiscountCash1 += parseFloat(value);
		}
	});
    $('.sumTotal1').each(function(index) {
        var value = $(this).attr('data-val');
        if (!isNaN(value) && value.length != 0) {
            sumTotal1 += parseFloat(value);
        }
    });
    $('.sumClaim1').each(function(index) {
        var value = $(this).attr('data-val');
        if (!isNaN(value) && value.length != 0) {
            sumClaim1 += parseFloat(value);
        }
    });
	$('#sumBill1').html(((sumBill1 == 0) ? '-' : numberWithCommas(sumBill1)));
    $('#sumCash1').html(((sumCash1 == 0) ? '-' : numberWithCommas(sumCash1)));
    $('#sumCredit1').html(((sumCredit1 == 0) ? '-' : numberWithCommas(sumCredit1)));
	$('#sumDiscountCash1').html(((sumDiscountCash1 == 0) ? '-' : numberWithCommas(sumDiscountCash1)));
    $('#sumTotal1').html(((sumTotal1 == 0) ? '-' : numberWithCommas(sumTotal1.toFixed(2))));
	$('#sumClaim1').html(((sumClaim1 == 0) ? '-' : numberWithCommas(sumClaim1)));
...
function loadCategory3() {
    ...
    $.post('https://api.remaxthailand.co.th/category/info', {
        ...
        if (data.success) {
            ...
            $('#cbb-catrgory3 option:eq(0)').attr('selected', 'selected');
            $('#cbb-catrgory3r option:eq(0)').attr('selected', 'selected');
            $('#cbb-catrgory3st option:eq(0)').attr('selected', 'selected');
            $('#dv-loading3').hide();
            $('#form-select3').show();
            $('#dv-loading3r').hide();
            $('#form-select3r').show();
    ...
    $.post('https://api.remaxthailand.co.th/shop/aging', {
        ...
        if (data.success) {
            ...
            for (i = 0; i < json.length; i++) {
                ...
                html += '<h6><smal class="pull-right text-muted">' + ((result.groupId == "19") ? '(ยกเลิกใช้งาน)' : ' ') + '</smal></h6></td>';
